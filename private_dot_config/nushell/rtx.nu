...
def-env rtx [command?: string, --help, ...rest: string] {
  ...
  if ($command == null) {
    ^"/Users/kean.mattingly@equipmentshare.com/.cargo/bin/rtx"
  } else if ($command == "activate") {
    let-env RTX_SHELL = "nu"
  } else if ($command in $commands) {
    ^"/Users/kean.mattingly@equipmentshare.com/.cargo/bin/rtx" $command $rest
    | parse vars
    | update-env
  } else {
    ^"/Users/kean.mattingly@equipmentshare.com/.cargo/bin/rtx" $command $rest
...
def-env rtx_hook [] {
  ^"/Users/kean.mattingly@equipmentshare.com/.cargo/bin/rtx" hook-env -s nu
    | parse vars
    | update-env
}
